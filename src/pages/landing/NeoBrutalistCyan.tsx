...
/**
 * Neo-Brutalist Landing Page - CYAN Variation
 * Color: Pure black + white + electric cyan (#00FFFF)
 */
...
function BrutalButton({ children, to, variant = 'primary', size = 'md', className = '' }: { children: React.ReactNode; to?: string; variant?: 'primary' | 'secondary' | 'outline'; size?: 'sm' | 'md' | 'lg'; className?: string }) {
  ...
  const inlineStyle = variant === 'primary' ? { backgroundColor: brutal.accent, color: brutal.black, borderColor: brutal.black } : variant === 'secondary' ? { backgroundColor: brutal.black, color: brutal.accent, borderColor: brutal.accent } : {};
    ...
    <span className={`relative inline-block ${className}`}>
      ...
      {isGlitching && (
        <>
          ...
          <span className="absolute inset-0" style={{ color: '#ff0000', transform: 'translate(2px, 2px)', clipPath: 'inset(40% 0 20% 0)' }}>{children}</span>
...
export default function NeoBrutalistCyan() {
